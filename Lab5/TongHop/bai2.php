...
<head>
	...
	<title>Phép tính trên phân số</title>
</head>
...
<body>
	...
	class PHAN_SO
	{
		...
		function thuong($p_tuso, $p_mauso)
		{
			...
			$ps->khoitao_ps($p_tuso, $p_mauso);
			$temp = new PHAN_SO();
			$temp->khoitao_ps($p_tuso, $p_mauso);
			$ps->tuso = $this->tuso * $temp->mauso;
			$ps->mauso = $this->mauso * $temp->tuso;
			if($ps->tuso != 0) {
				$ps->toigian_ps();
			}
			return $ps;
	...
	<form id="form1" name="form1" method="post" action="">
		...
		<p>Nhập phân số thứ 1: Tử số:
			<input name="tuso_1" type="text" id="tuso_1" size="10" maxlength="10" value="<?php echo $tuso_1 ?>" pattern="[-]*[0-9]*"/>
			Mẫu số:
			<input name="mauso_1" type="text" id="mauso_1" size="10" maxlength="10" value="<?php echo $mauso_1 ?>" pattern="[-]*[0-9]*"/>
		</p>
		<p>Nhập phân số thứ 2: Tử số:
			<input name="tuso_2" type="text" id="tuso_2" size="10" maxlength="10" value="<?php echo $tuso_2 ?>" pattern="[-]*[0-9]*"/>
			Mẫu số:
			<input name="mauso_2" type="text" id="mauso_2" size="10" maxlength="10" value="<?php echo $mauso_2 ?>" pattern="[-]*[0-9]*"/>
		</p>
		<fieldset>
			...
			<p>
				...
				<label>
					<input type="radio" name="pheptinh" value="nhân" <?php if (isset($_POST['pheptinh']) && ($_POST['pheptinh']) == "nhân") echo 'checked' ?> />
					Nhân
				</label>

				<label>
					<input type="radio" name="pheptinh" value="chia" <?php if (isset($_POST['pheptinh']) && ($_POST['pheptinh']) == "chia") echo 'checked' ?> />
					Chia
				</label>
			</p>
		</fieldset>
	...
	if (isset($_POST['Chon_pheptinh'])) {
		...
		switch ($pheptinh) {
			...
			case "nhân":
				...
				$ketqua = $ps_1->get_tuso() . "/" . $ps_1->get_mauso() . "x" . $ps_2->get_tuso() . "/" . $ps_2->get_mauso() . "=" . $ps->get_tuso() . "/" . $ps->get_mauso();
				break;
			case "chia":
				$ps = new PHAN_SO();
